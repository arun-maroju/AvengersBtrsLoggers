...
public class AdminFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("IN admin do filter.. method..");
		...
		if (path.equals("/adminLogin") || path.equals("/logout")) {
		...
		if (httpRequest.getSession().getAttribute("adminLogged") != null) {

			System.out.println("IN admin do filter.. method.. if block2");

			// User is authenticated, allow access to requested page
			chain.doFilter(request, response);
		} else {
			System.out.println("IN admin do filter.. method.. if block2 else block");
